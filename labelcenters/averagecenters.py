...
def averageCenters(input_ar, input_nb):
    df_ar = pd.read_csv(input_ar)
    df_nb = pd.read_csv(input_nb)
    df_average = df_ar
    for (clickname, point) in df_ar.iteritems():
        df_average[clickname] = calculateMidpoint(df_ar[clickname], df_nb[clickname])
    return df_average  
...
def main(input_image, show_image):
    # reading the image
    ...
    df = averageCenters(input_ar, input_nb)
...
if __name__=="__main__":
    ...
    show_image = input("Would you like to see the image? 0 if yes, 1 if no. \n")
    main(input_image, show_image)