...
def makeFixationFeaturePlot(trial_featmap, fixations_block_trial, block_centers, indices, labels, xdim, ydim, height=3, ticks=False, cmap=None):
    ## Initialize plot.
...
def plotAOIFeatureFixations(subj_id, block, trial):
    ## Load data.
    data, raw_pos_data, messages, sfreq = load_subj_data(subj_id)
    ...
    return makeFixationFeaturePlot(this_trial_featmap, fixations_block_trial, block_centers, indices, labels, xdim, ydim)
